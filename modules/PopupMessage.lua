...
L:RegisterTranslations("enUS", function() return {
    ["PopupMessage"] = true,
    ["Shows messages with certain keywords in a popup."] = true,
    ["Toggle"] = true,
    ["Show"] = true,
    ["ShowAll"] = true,
    ...
    ["Nickname"] = true,
    ["Words"] = true,
    ["See list of keywords."] = true,
    ["KeywordClear"] = true,
    ["Clear list of keywords."] = true,
} end)

L:RegisterTranslations("zhCN", function() return {
    ["PopupMessage"] = "弹出消息",
    ["Shows messages with certain keywords in a popup."] = "Shows messages with certain keywords in a popup.",
	["Toggle"] = "切换",
    ...
    ["Nickname"] = "昵称",
    ["Words"] = "Words",
    ["See list of keywords."] = "See list of keywords",
    ["KeywordClear"] = "KeywordClear",
    ["Clear list of keywords."] = "Clear list of keywords.",
} end)
...
function Prat_PopupMessage:OnInitialize()
    ...
    Prat.Options.args.PopupMessage = {
        name = L["PopupMessage"],
        desc = L["Shows messages with certain keywords in a popup."],
        type = "group",
        args = {
            Toggle = {
                type = "toggle",
            ...
            keyword = {
                name = L["Words"],
                desc = L["See list of keywords."],
                type = "text",
                order = 90,
                get = function() 
                    local wordString = ""
                    for keyword in self.db.profile.keywords do
                        wordString = wordString..keyword..", "
                    end
                    wordString = string.sub(wordString, 1, -3) -- remove the final ", "
                    return wordString
                end,
                set = function(w)
                    -- Remove the word if it is there already, else add it
                    if self.db.profile.keywords[w] then
                        self.db.profile.keywords[w] = nil
                    else
                        self.db.profile.keywords[w] = true
                    end
                end
            },
            keywordClear = {
                name = L["KeywordClear"],
                name = L["Clear list of keywords."],
                order = 105,
                type = "text",
                get = function()
                    self.db.profile.keywords = {}
                end,
                set = function() end
            },
            show = {
...
function Prat_PopupMessage:AddMessage(frame, text, r, g, b, id)
    local textWords = Prat.stringTokenize(text)

    for i=1, table.getn(textWords) do
        local word = textWords[i]
        if self.db.profile.keywords[word] then
    ...
    if Name ~= self.playerName then 
        ...
        -- Ignore certain cases a player's name is mentioned:
        local _, n = string.gsub(textL, playerL .. " rolls %d+ %(%d+-%d+%)", "")
        if n ~= 0 then
            return
        end
