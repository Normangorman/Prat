...
-- Begin Gnoggin mod
function Prat_PlayerNames:updateSpecificPlayer(PlayerName)
    SendWho(PlayerName)
end
-- end Gnoggin mod
...
function Prat_PlayerNames:AddMessage(frame, text, r, g, b, id)
    if text and not (string.find(text, "_Prat_")) then
        local Name = string.gsub(text, ".*|Hplayer:(.-)|h.*", "%1")
        local Brackets

        -- Begin Gnoggin mod
        if Name ~= text then -- A player spoke
            local update = true
            --DEFAULT_CHAT_FRAME:AddMessage("_Prat_Player spoke: "..Name)

            -- Check if player is in guild, is a friend, or is self --
            -- in which case don't update
            for i = 1, GetNumGuildMembers() do
                local guild_name, _, _, _, _ = GetGuildRosterInfo(i)
                if guild_name == Name then
                    update = false
                    --DEFAULT_CHAT_FRAME:AddMessage("_Prat_Player is guildie")
                    break
                end
            end
            ...
            if update then
                for i = 1, GetNumFriends() do
                    local friend_name, _, _ = GetFriendInfo(i)
                    if friend_name == Name then
                        update = false
                        --DEFAULT_CHAT_FRAME:AddMessage("_Prat_Player is friend")
                        break
                    end
                end
            end
            ...
            if update then
                if current_time - LastUpdateTime < 2.0 then -- wait 2 secs between tries to prevent spam
                    update = false
                    --DEFAULT_CHAT_FRAME:AddMessage("_Prat_Too frequent")
                end
            end

            if update then
                local last_update_time = UpdateTimes[Name]
                --DEFAULT_CHAT_FRAME:AddMessage("_Prat_Current time: "..current_time)
                if last_update_time then
                    local delta = current_time - last_update_time
                    --DEFAULT_CHAT_FRAME:AddMessage("_Prat_Last update time: "..last_update_time)
                    --DEFAULT_CHAT_FRAME:AddMessage("_Prat_Delta: "..delta)

                    if delta > 600 then -- 10 minutes
                        --DEFAULT_CHAT_FRAME:AddMessage("_Prat_Updating for player...")
                        Prat_PlayerNames:updateSpecificPlayer(Name)
                        UpdateTimes[Name] = current_time
