...
function Prat.stringTokenize(s, pat)
    pat = pat or "[^%s]+"
    local tokens = {}

    local string_cursor = 1
    while true do
        local match_s, match_e = string.find(s, pat, string_cursor)
        if match_s then
            local token = string.sub(s, match_s, match_e)
            table.insert(tokens, token)
            string_cursor = match_e + 1
        else
            break
        end
    end
